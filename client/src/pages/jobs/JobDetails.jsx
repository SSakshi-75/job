...
const JobDetails = () => {
    ...
    if (loading) {
        return (
            <div className="flex flex-col justify-center items-center min-h-[calc(100vh-64px)] gap-4 transition-colors">
        ...
        return (
            <div className="max-w-xl mx-auto px-4 py-20 text-center">
                <div className="bg-white rounded-3xl border border-slate-100 p-12 shadow-sm">
                    ...
                    <button
                        onClick={() => navigate(-1)}
                        className="flex items-center gap-2 mx-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-indigo-500/20"
                    >
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-6 md:py-10">
            ...
            {/* ── Hero Card ── */}
            <div className="bg-white rounded-3xl border border-slate-200/70 p-6 md:p-10 mb-6 shadow-sm relative overflow-hidden transition-colors">
                {/* Gradient blob */}
                ...
                {/* Stats Row */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8 pt-8 border-t border-slate-100 transition-colors">
                    {[
                        ...
                        <div key={label} className="bg-slate-50 rounded-2xl p-4 border border-slate-100 transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-3xl border border-slate-200/70 p-6 md:p-10 shadow-sm transition-colors">
                ...
                <div className="space-y-5">
                    ...
                    {/* AI Interview Prep Coach (Unique Feature) */}
                    {user?.role === "job-seeker" && (
                        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h3 className="text-white font-black text-sm uppercase tracking-tight">AI Prep Coach</h3>
                                </div>
                                <p className="text-indigo-100/70 text-[12px] leading-relaxed mb-5 font-medium">
                                    Our AI has analyzed this job description. Get ready with these personalized interview questions:
                                </p>
                                <div className="space-y-3">
                                    {[
                                        `Explain your experience with ${job.skills?.[0] || 'core technologies'}.`,
                                        "How do you handle tight deadlines in a team?",
                                        `Why do you want to join ${job.company}?`
                                    ].map((q, i) => (
                                        <div key={i} className="p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors cursor-default">
                                            <p className="text-white/90 text-[11px] font-bold italic leading-snug">"{q}"</p>
                                        </div>
                                    ))}
                                </div>
                                <button className="w-full mt-5 py-3 bg-indigo-500 hover:bg-indigo-400 text-white text-[11px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-indigo-500/20">
                                    Unlock Full Mock Interview
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {/* Company Card */}
                    <div className="bg-slate-900 rounded-3xl p-6 relative overflow-hidden shadow-xl transition-colors">
