...
const statusConfig = {
    pending:  { label: "Pending Review",  icon: Clock,    bg: "bg-amber-50",   text: "text-amber-600",   border: "border-amber-100" },
...
const AppliedJobs = () => {
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center min-h-[calc(100vh-64px)] bg-[#F8F7F2] transition-colors">
    ...
    return (
        <div className="min-h-screen bg-[#F8F7F2] pb-20 transition-colors">
            {/* Header */}
            ...
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                    {[
                        ...
                        <div key={s.label} className={`${s.bg} rounded-3xl p-6 border border-slate-100 shadow-sm transition-colors`}>
                            <div className={`text-3xl font-black ${s.color} mb-1 transition-colors`}>{s.count}</div>
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest transition-colors">{s.label}</div>
                        </div>
                    ))}
                </div>
                ...
                {/* Application Cards */}
                {filtered.length === 0 ? (
                    <div className="bg-white rounded-[32px] border border-slate-100 p-20 text-center shadow-sm transition-colors">
                        <Briefcase className="w-16 h-16 text-slate-200 mx-auto mb-4" />
                        <h2 className="text-2xl font-black text-slate-900 mb-2 transition-colors">No Applications Found</h2>
                        <p className="text-slate-500 font-medium mb-8 transition-colors">Start applying to great opportunities that match your skills.</p>
