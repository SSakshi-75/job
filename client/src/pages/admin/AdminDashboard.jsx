...
import { 
    Users, Briefcase, FileText, Building, 
    Trash2, Shield, Search, ChevronDown, 
    ToggleLeft, ToggleRight, Loader2, Video, 
...
const AdminDashboard = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [sRes, uRes, jRes, aRes, iRes] = await Promise.all([
                api.get("/admin/stats"),
                api.get("/admin/users"),
                api.get("/admin/jobs"),
            ...
            const formatChartData = (data, keyName) => {
            ...
            setStats({
                ...sRes.data.data,
                charts: {
                    userGrowth: formatChartData(sRes.data.data.charts.userGrowth || [], "Users"),
                    jobVolume: formatChartData(sRes.data.data.charts.jobVolume || [], "Jobs")
                }
            });
    ...
    const handleDeleteUser = async (id) => {
        if (!window.confirm("Permanent deletion? This user will be purged from the system.")) return;
        try {
    ...
    const handleToggleBan = async (id, currentlyBanned) => {
        try {
            await api.patch(`/admin/users/${id}/ban`, { isBanned: !currentlyBanned });
            setUsers(users.map(u => u._id === id ? { ...u, isBanned: !currentlyBanned } : u));
    ...
    const handleToggleJobStatus = async (id) => {
        try {
            ...
            setJobs(jobs.map(j => j._id === id ? { ...j, status: j.status === 'active' ? 'inactive' : 'active' } : j));
    ...
    const TABS = [
        ...
        { id: "applications", label: "Apps", icon: <FileText className="w-4 h-4" /> },
        { id: "interviews", label: "Interviews", icon: <Video className="w-4 h-4" /> },
    ];

    return (
        <div className="min-h-screen bg-[#FDFCF8] pb-20 transition-colors" onClick={() => setRoleDropdown(null)}>
            <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                    ...
                    <div className="hidden lg:flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-4">
                            {[
                                { icon: FileText, text: `${stats.metrics.totalApplications} Apps`, color: "text-violet-500" },
                                { icon: Calendar, text: `${interviews.length} Interviews`, color: "text-amber-500" }
                            ].map((pills, i) => (
                                <div key={i} className="px-4 py-2 bg-white border border-slate-100 rounded-2xl shadow-sm flex items-center gap-2">
                                    <pills.icon className={`w-4 h-4 ${pills.color}`} />
                                    <span className="text-xs font-black text-slate-700">{pills.text}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {activeTab === "overview" && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {[
                                { label: "Total Users",    value: stats.metrics.totalUsers,        icon: <Users className="w-7 h-7" />,    color: "bg-blue-50 text-blue-600", tab: "users" },
                                { label: "Employers",      value: stats.metrics.totalRecruiters,   icon: <Building className="w-7 h-7" />, color: "bg-indigo-50 text-indigo-600", tab: "users" },
                                { label: "Live Jobs",      value: stats.metrics.totalJobs,         icon: <Briefcase className="w-7 h-7" />,color: "bg-emerald-50 text-emerald-600", tab: "jobs" },
                                { label: "Applications",   value: stats.metrics.totalApplications, icon: <FileText className="w-7 h-7" />, color: "bg-rose-50 text-rose-600", tab: "applications" },
                            ].map((kpi) => (
                                <div 
                                    key={kpi.label} 
                                    onClick={() => setActiveTab(kpi.tab)}
                                    className="bg-white p-7 rounded-[40px] border border-slate-100 shadow-sm hover:shadow-2xl hover:translate-y-[-4px] transition-all duration-500 cursor-pointer group"
                                >
                                    <div className={`w-14 h-14 ${kpi.color} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500`}>{kpi.icon}</div>
                                    ...
                                    <div className="h-[280px]">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <AreaChart data={chart.data}>
                                                ...
                                                <Tooltip contentStyle={{borderRadius: '20px', border: 'none', boxShadow: '0 20px 50px rgba(0,0,0,0.1)', fontWeight: 900}} />
                                                <Area type="monotone" dataKey={chart.dataKey} stroke={chart.color} strokeWidth={4} fillOpacity={1} fill={`url(#${chart.grad})`} />
                                            </AreaChart>
                                        </ResponsiveContainer>
                                    </div>
                                </div>
                            ...
                            <div className="p-10 border-b border-slate-50 flex flex-col sm:flex-row gap-6 items-center justify-between">
                                ...
                                <div className="relative w-full sm:w-72">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-11 pr-4 py-3.5 bg-slate-50 rounded-2xl text-[11px] font-black text-slate-900 outline-none border border-transparent focus:border-blue-500/20 transition-all shadow-inner"
                                    />
                                </div>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {users.filter(u => 
                                            ...
                                            <tr key={user._id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-10 py-6">
                                                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${user.isBanned ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-emerald-50 text-emerald-600 border-emerald-100"}`}>
                                                ...
                                                <td className="px-10 py-6">
                                                    <div className="flex items-center gap-2 justify-end">
                                                        <button 
                                                            ...
                                                            className={`p-2 rounded-lg transition ${user.isBanned ? "bg-emerald-600 text-white shadow-lg shadow-emerald-600/20" : "bg-slate-100 text-slate-400 hover:bg-rose-600 hover:text-white"}`}
                                                        ...
                                                        <button 
                                                            ...
                                                            className="p-2 bg-slate-100 text-slate-400 hover:bg-rose-600 hover:text-white rounded-lg transition"
                                                            title="Purge User"
                                                        >
                        ...
                        <div className="bg-white rounded-[48px] border border-slate-100 shadow-sm overflow-hidden">
                                ...
                                <div className="flex gap-2 w-full sm:w-auto">
                                    ...
                                    <div className="relative flex-1 sm:w-64">
                                        ...
                                        <input 
                                            ...
                                            className="w-full pl-11 pr-4 py-2.5 bg-slate-50 rounded-2xl text-[11px] font-black text-slate-900 outline-none border border-transparent focus:border-blue-500/20 transition-all shadow-inner"
                                        />
                                    </div>
                                </div>
                            </div>
                            <div className="divide-y divide-slate-50">
                                {jobs.filter(j => {
                                    ...
                                    const matchSuspicious = showSuspiciousOnly ? isJobSuspicious(j) : true;
                                            ...
                                            <tr key={app._id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-10 py-5 text-right">
                                                    <button 
                                                        ...
                                                        onClick={() => alert("Detailed view pending")}
                                                    >
                                                        Details
                                                    </button>
                                                </td>
                ...
                {activeTab === "interviews" && (
                    <div className="animate-in slide-in-from-bottom-4 duration-700">
                        <div className="bg-white rounded-[48px] border border-slate-100 shadow-sm overflow-hidden">
                            <div className="p-10 border-b border-slate-50 transition-colors">
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {interviews.map((intv) => (
                                            <tr key={intv._id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-10 py-5 text-right">
                                                    <button 
                                                        className="px-3 py-1.5 bg-slate-900 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 transition"
                                                        onClick={() => window.open(intv.link, "_blank")}
                                                    >
                                                        Join
                                                    </button>
                                                </td>
