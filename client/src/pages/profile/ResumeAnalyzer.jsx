                ...
                {result && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="flex flex-wrap gap-2 max-h-[120px] overflow-y-auto pr-2 custom-scrollbar">
                                    {result.matched?.length > 0 ? (
                                        ...
                                        <div className="text-slate-400 text-xs italic space-y-2">
                                            ...
                                            <p className="text-[10px] opacity-70">Tip your resume is not just a scanned image.</p>
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* UNIQUE FEATURE Salary & Negotiation (Full Width below) */}
                            <div className="md:col-span-2 bg-gradient-to-br from-[#0F172A] to-[#1E293B] rounded-[40px] p-8 md:p-12 shadow-2xl relative overflow-hidden group">
                                <div className="absolute top-0 right-0 w-96 h-96 bg-blue-500/10 rounded-full blur-[100px] group-hover:bg-blue-500/20 transition-all duration-700" />
                                <div className="relative z-10 flex flex-col lg:flex-row items-center gap-10">
                                    <div className="text-center lg:text-left space-y-4 lg:max-w-md">
                                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-500/20">
                                            Market Insight
                                        </div>
                                        <h3 className="text-3xl font-black text-white">Salary <span className="text-blue-400">Negotiation</span> AI</h3>
                                        <p className="text-slate-400 font-medium text-sm leading-relaxed">
                                            Based on your detected seniority and {result.matched?.[0] || "skills"}, here is your personalized market positioning.
                                        </p>
                                    </div>

                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                                        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:bg-white/10 transition-all">
                                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Estimated Market Value</p>
                                            <div className="text-3xl font-black text-white">₹8.5L - ₹14L</div>
                                            <p className="text-[11px] text-emerald-400 font-bold mt-2">↑ 15% above average for this role</p>
                                        </div>
                                        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:bg-white/10 transition-all">
                                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Negotiation Strategy</p>
                                            <p className="text-white text-xs font-bold leading-relaxed italic">
                                                "Focus on your expertise in {result.matched?.[0] || 'Domain Knowledge'} to justify a premium package. High confidence detected in technical strengths."
                                            </p>
                                        </div>
                                    </div>
                                </div>
