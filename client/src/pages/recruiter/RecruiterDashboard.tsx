...
const RecruiterDashboard = () => {
    ...
    const pendingApps = recentApps.filter((a: any) => a.status === "pending").length;
    const upcomingInterviews = interviews.filter((i: any) => new Date(i.scheduledAt) > new Date()).length;
    ...
    if (loading) {
        return (
            <div className="flex flex-col justify-center items-center min-h-[calc(100vh-64px)] gap-3">
    ...
    return (
        <div className="min-h-screen bg-slate-50/50">
            ...
            {/* ── Hero Header ── */}
            <div className="bg-white border-b border-slate-200/80 px-4 py-8 md:py-10">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8 md:py-10 space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* Recent Applications */}
                    <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200/70 shadow-sm overflow-hidden">
                        ...
                        <div className="divide-y divide-slate-50">
                            {recentApps.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-14 text-center px-6">
                                    ...
                                    <p className="font-bold text-slate-700 mb-1">No applications yet</p>
                                    <p className="text-slate-400 text-sm font-medium mb-4">Post a job to start receiving candidates</p>
                                    <Link
                                        to="/post-job"
                                ...
                                recentApps.map((app: any) => (
                                    <div key={app._id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/60 transition-colors group">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[11px] text-slate-400 font-medium truncate">{app.job?.title || "Position"}</p>
                                        </div>
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="bg-white rounded-3xl border border-slate-200/70 p-6 shadow-sm">
                            <h2 className="text-sm font-black text-slate-900 uppercase tracking-tight mb-4">Quick Actions</h2>
                        ...
                        {jobs.length > 0 && (
                            <div className="bg-white rounded-3xl border border-slate-200/70 p-6 shadow-sm">
                                <div className="flex items-center justify-between mb-4">
                                    <h2 className="text-sm font-black text-slate-900 uppercase tracking-tight">Active Listings</h2>
                                    <Link to="/my-jobs" className="text-[10px] font-black text-indigo-500 uppercase tracking-widest hover:underline">All →</Link>
                                </div>
                                <div className="space-y-2.5">
                                    {jobs.filter((j: any) => j.status === "active").slice(0, 3).map((job: any) => (
